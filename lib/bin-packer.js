...
function firstFitArray(obj, sizeOf, capacity, presort) {
  const array = utils.validateIterable(utils.toArray(obj), sizeOf)
      , bins = []
      , oversized = []
      , remaining = []

  if (presort) {
    utils.sortDecreasing(array, sizeOf)
  }
...
function bestFitDecreasing(obj, sizeOf, capacity) {
  ...
  const sorted = utils.sortDecreasing(utils.validateIterable(utils.toArray(obj), sizeOf), sizeOf)
      , bins = []
      , oversized = []
...
function lowerBound1(obj, sizeOf, capacity) {
  return Math.ceil(utils.sumAndValidate(utils.toArray(obj), sizeOf) / capacity)
}
...
function lowerBound2(obj, sizeOf, capacity) {
  const sortedArray = utils.sortDecreasing(utils.validateIterable(utils.toArray(obj), sizeOf), sizeOf)
  let waste = 0
      , carry = 0
