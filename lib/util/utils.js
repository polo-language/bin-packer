...
/**
 * Throws if {@link toNumber} does not return a number for any element of {@link iterable}.
 * Returns {@link iterable} for chaining.
 * @param {iterable} iterable 
 * @param {function} toNumber 
 */
function validateIterable(iterable, toNumber) {
  for (const item of iterable) {
    validateNumber(toNumber(item))
  }
  return iterable
}
...
function validateNumber(num) {
  if (num === null || num === undefined || typeof num !== 'number') {
    throw new Error(`sizeOf did not return a number`)
  }
  return num
}

function sortDecreasing(array, sizeOf) {
...
function sumAndValidate(array, sizeOf) {
  return array.reduce((acc, cur) => acc += validateNumber(sizeOf(cur)), 0)
}
