...
module.exports = {
  prepareValues,
  prepareValuesNoCapacity,
  toArray,
  isIterable,
  adaptCapacityToIterable,
  sortDescending,
  sortAscending,
  sum,
  binaryApply,
...
/**
 * Converts its argument to an iterable.
 * The type of values returned by the iterable isn't checked here so as to not consume any values.
 * @param {iterable|function|number} capacity
 */
function adaptCapacityToIterable(capacity) {
  ...
  throw new Error('Capacity must be one of: iterable, generator function, or number')
}
